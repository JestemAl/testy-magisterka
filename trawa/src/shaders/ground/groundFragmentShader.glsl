...
void main() {
  ...
  // Grid
  float grid1 = texture(groundTexture, vWorldPosition.xz * 0.1).r;
	float grid2 = texture(groundTexture, vWorldPosition.xz * 1.0).r;
  ...
  // vec3 colour = vec3(gridColour);
  vec3 color = vec3(0.2, 0.12, 0.05);
